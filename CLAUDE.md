...
- **Routing**: [react-router-dom](https://reactrouter.com/) v7, via `<BrowserRouter>` in `src/main.tsx` wrapping `<App>`. `App.tsx` defines the 5 tab paths (`/`, `/pokoje`, `/plan`, `/ustawienia`, `/dodaj`) plus nested full-page routes like `/ustawienia/sklepy/nowy` and `/ustawienia/sklepy/:shopId` ([ShopFormPage](src/pages/ShopFormPage.md)) that intentionally hide the bottom tab bar (checked via `useLocation().pathname` in `AppShell`) for focused add/edit flows reached from a settings list. Use `<Link>`/`useNavigate`/`useParams` for navigation; when putting a router `Link` inside a Chakra component, use `asChild` (e.g. `<Button asChild><Link to="...">...</Link></Button>`) rather than Chakra's `as={Link}` polymorphic prop — the latter doesn't type-check `to`.
  - **Desktop modal routes**: on `md`+ viewports (see [useIsDesktop](src/hooks/useIsDesktop.md)), the add/edit routes (shop/room forms, plan item form, invoice form) render as a Chakra `Dialog` over the current page instead of navigating away, via the "background location" pattern in `src/lib/modalRoute.ts` + `App.tsx`'s `AppShell`/`ModalRouteOverlay`: any `Link`/`navigate()` call that should open one of these routes as a dialog spreads `withBackground(location)` into its `state`/options (see `ShopsSection`, `RoomsSection`, `AllPlanItemsPage`, `RoomDetailPage`, `SidebarNav`'s `modal: true` items). `AppShell` reads `location.state.backgroundLocation` — but only when `useIsDesktop()` is `true` — and renders the base `<Routes>` at that background location (so the underlying page keeps showing) plus a second `<Routes>` for just the modal-eligible paths (`MODAL_ROUTES`) inside the dialog at the actual location. On mobile the state is ignored entirely, so the exact same `Link`/`navigate()` calls just navigate to a full page as before — no separate mobile/desktop navigation code paths. Closing the dialog (backdrop/Escape) navigates back to the background location; the form pages' own back arrows already `navigate(-1)`/a fixed path, which works unchanged in both modes.
...
- [SidebarNav](src/components/SidebarNav.md)
...
- [useIsDesktop](src/hooks/useIsDesktop.md) (hook, not a component, but documented the same way)
